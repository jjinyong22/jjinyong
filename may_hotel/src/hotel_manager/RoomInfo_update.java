...
class RoomModel2 extends AbstractTableModel{
	...
	RoomModel2(RoomInfo_update room_update){
...
class LoadActionListener2 implements ActionListener {

	RoomInfo_update panel;
	BackImage2 image;
...
//	EmployeeInfo_main main;
	...
	LoadActionListener2(RoomInfo_update panel, BackImage2 image, hotel_customer.Screen main){
		this.panel = panel;
		this.image = image;
		this.main = main;
	}
...
class SaveActionListener2 implements ActionListener{
...
//	EmployeeInfo_main main;
	...
	SaveActionListener2(RoomInfo_update panel, hotel_customer.Screen main){
		this.panel = panel;
		this.main = main;
	}
...
public class RoomInfo_update extends JPanel implements ActionListener, MouseListener{
	
	JLabel[] label; 
	String[] label_txt = {"객실 정보 변경", "객실 선택", "객실 호수", "호","예약 금액",
	...
	ImageIcon room_road, room_road_press, room_road_rollover, select, select_press, select_rollover,
				  ...
				  update, update_press, update_rollover, cancel, cancel_press, cancel_rollover;
...
//	EmployeeInfo_main main;
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (obj == btns[0]) {
			table.setModel(new RoomModel2(this));
		}
		...
		if(obj == btns[3]) {
			if(price_text.getText().equals("")) {
				...
				String[] str = {"변 경","취 소"}; 
				int select = JOptionPane.showOptionDialog(this, "변경하시겠습니까?", "예 약", JOptionPane.YES_NO_OPTION, JOptionPane.INFORMATION_MESSAGE, null, str, str[0]); 
				if(select == JOptionPane.YES_OPTION) {
					CRUDprocess_emp crud = new CRUDprocess_emp();
					RoomInfo info = new RoomInfo();
					String room_type = (String)room_combo.getSelectedItem();
					int room_id = Integer.parseInt(room_idText.getText());
					int room_price = Integer.parseInt(price_text.getText());
					int room_head = head_combo.getSelectedIndex()+1;
					String room_check = null;
					if(res_ok.isSelected()) {
						room_check = "Y";
					}else if(res_no.isSelected()) {
						room_check = "N";
					}
					...
					int result = crud.updateRoomInfo(info);

					if(result > 0) {
						JOptionPane.showMessageDialog(this, "객실정보가 변경되었습니다.");
						...
						JOptionPane.showMessageDialog(this, "객실정보 변경과정에서 문제가 발생했습니다.");
...
//	EmployeeInfo_main main
	...
	//super(str);
		...
		image = new BackImage2();
		
		btns[1].addActionListener(new LoadActionListener2(this, image, main));
		
		btns[2].addActionListener(new SaveActionListener2(this, main));
