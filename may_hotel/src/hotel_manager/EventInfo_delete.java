...
class EventModel3 extends AbstractTableModel {

	private Object[][] tableData;
	private int cols, rows;
	private String[] columnName = { "이벤트번호", "이벤트이름", "이벤트내용", "이벤트시작일", "이벤트종료일", "이벤트이미지경로" };
	private List<EventInfo> list;
	EventInfo_delete event_update;

	EventModel3(EventInfo_delete event_update) {
...
class BackImage_event3 extends JPanel {
	Image img;
...
class LoadActionListener_event3 implements ActionListener {

	EventInfo_delete panel;
	BackImage_event3 image;
...
//	EmployeeInfo_main main;
	...
	LoadActionListener_event3(EventInfo_delete panel, BackImage_event3 image, hotel_customer.Screen main) {
		this.panel = panel;
		this.image = image;
		this.main = main;
	}
...
class SaveActionListener_event3 implements ActionListener {
	FileDialog fileDialog;
	EventInfo_delete panel;
...
//	EmployeeInfo_main main;
	...
	SaveActionListener_event3(EventInfo_delete panel, hotel_customer.Screen main) {
		this.panel = panel;
		this.main = main;
	}
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		fileDialog.setFile(LoadActionListener_event3.fileName);
		fileDialog.setVisible(true);
		...
		try {
			...
			int data = 0;
			while ((data = LoadActionListener_event3.bis.read()) != -1) {
				bos.write(data);
			}
		} catch (Exception ex) {
			...
			try {
				bos.close();
				LoadActionListener_event3.bis.close();
			} catch (Exception exe) {
...
public class EventInfo_delete extends JPanel implements ActionListener, MouseListener {
	JLabel labels;
	JLabel[] label;
	...
	String[] label_txt = { "이벤트 정보 삭제", "년", "월", "제 목", "이벤트 오픈날짜", "이벤트 종료날짜" };

	JLabel imageLabel;

	JTextField imagePath;

	ImageIcon event_road, event_road_press, event_road_rollover, choice, choice_press, choice_rollover, delete,
			delete_press, delete_rollover, cancel, cancel_press, cancel_rollover;
...
//	EmployeeInfo_main main;
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (obj == btns[0]) {
			table.setModel(new EventModel3(this));
		}
		if (obj == btns[3]) {
			if (textfield.getText().equals("")) {
				...
				if (!textfield2.getText().equals("") && !textfield3.getText().equals("")) {
					...
					if (select == JOptionPane.YES_OPTION) {
						...
						CRUDprocess_emp crud = new CRUDprocess_emp();
						int result = crud.deleteEventInfoByEventId(Integer.parseInt(id));
						if (result > 0) {
							JOptionPane.showMessageDialog(this, "이벤트 정보가 삭제되었습니다.");
							textfield.setText(" ");
							textfield.setText("");
							...
							imagePath.setText(" ");
							imagePath.setText("");
							imagePath.setEnabled(true);
							image.removeAll();
						} else {
							JOptionPane.showMessageDialog(this, "이벤트 삭제 과정에서 문제가 발생했습니다.");
						}
					}
				} else {
	...
	void makeButton() {
		...
		btns[5].addActionListener(new LoadActionListener_event3(this, image, main));
		...
		btns[6].addActionListener(new SaveActionListener_event3(this, main));
	}
	...
	EventInfo_delete(hotel_customer.Screen main) {
		// super(str);
	...
	// new EventInfo_update("이벤트 정보 화면 ver1.0");
	// }
}
