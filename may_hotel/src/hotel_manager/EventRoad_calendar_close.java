...
public class EventRoad_calendar_close extends JFrame implements WindowListener, ActionListener, ItemListener{
	...
	public EventRoad_calendar_close(EventInfo_insert event_insert){
		this.event_insert = event_insert; 
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		event_insert.textfield3.setText(res_date);
		dispose();//컴포넌트의 이벤트를 통해 윈도우 종료(메모리에서 삭제됨)
//		this.setDefaultCloseOperation(DISPOSE_ON_CLOSE); --> 상수로 구현.(윈도우의 x버튼을 통해서 종료 이벤트를 처리할 때 사용.(기능은 똑같음)
	}
	public void init(){
		select();
		calendar();
